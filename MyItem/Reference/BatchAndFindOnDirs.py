...
def BatchAndFindOnDirs(Dir,Fun):
    ...
    if os.path.isdir(Dir):
        while 1:
            Find = eval(input ("What want to find on this folder? "))
            if not Find.strip():
        ...
        if 'Rename' in Fun:
            while 1:
                Replace = eval(input("Please input new name to replace: "))
        ...
        if 'Del' in Fun:
            while 1:
                IsDel = eval(input("Are you sure to batch delete files or folders? (Y/N)"))
                if IsDel.lower() == 'y':
                    break
                elif IsDel.lower() == 'n':
                    CountineOrExit()
        ...
        for root,dirs,filenames in os.walk(Dir): 
            ...
            print "path: " + aa
            print "filename: " +bb
            print "Full path is : "+ os.path.join(aa,bb)
            print "    "
            ...
            if Find.lower() in aa.lower():
                FD = dirs
            """
            FTypes = [dirs,filenames]
            for FD in FTypes:
                for myFile in FD:
...
if __name__ == '__main__':
    Dir  = eval(input ("Please input folder path: "))
